...
```bash
curl --silent --location --max-redirs 10 https://raw.githubusercontent.com/SliMM/romanian-programmers-mac/master/install.sh --output "/tmp/rpm-install.sh" && bash "/tmp/rpm-install.sh" && rm -f "/tmp/rpm-install.sh"
```
